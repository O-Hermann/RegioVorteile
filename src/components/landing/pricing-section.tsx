...
export function PricingSection() {
  return (
    <section id="preise" className="py-20 bg-white dark:bg-slate-950">
      <div className="mx-auto max-w-6xl px-4 sm:px-6">
        ...
        <div className="mt-12 grid gap-6 sm:grid-cols-3">
          {TIERS.map((tier) => (
            <div
              key={tier.name}
              className="flex flex-col rounded-2xl border border-petrol-100 dark:border-white/10 bg-white dark:bg-slate-900 p-6 shadow-sm"
            >
              ...
              <p className="mt-5 font-display text-2xl font-extrabold text-petrol-800 dark:text-cyan-300">
                Preis folgt
              </p>
              ...
              <ul className="mt-5 space-y-2 text-sm text-slate-700 dark:text-slate-300">
                {tier.features.map((feature) => (
                  <li key={feature} className="flex gap-2.5">
                    <span className="mt-0.5 text-petrol-700 dark:text-cyan-400">+</span>
                    {feature}
                  </li>
                ))}
              </ul>
